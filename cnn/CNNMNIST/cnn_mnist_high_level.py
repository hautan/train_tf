...
train_labels = np.asarray(mnist.train.labels, dtype=np.int32)#ValueError: Can not squeeze dim[1], expected a dimension of 1, got 10 for 'sparse_softmax_cross_entropy_loss/remove_squeezable_dimensions/Squeeze
...
eval_input_fn = tf.estimator.inputs.numpy_input_fn(x={"x": eval_data}, y=eval_classes, num_epochs=1, shuffle=False)
